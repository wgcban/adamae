...
OUTPUT_DIR='experiments/ssv2_videomae_pretrain_pretrain_videomae_base_patch16_224_learnable_masking_ratio_0.95_final/'
# Set the path to SSV2 train set. 
DATA_PATH='/data/wbandar1/datasets/ss2/something-something-v2-train.csv'

# batch_size can be adjusted according to number of GPUs
# this script is for 64 GPUs (8 nodes x 8 GPUs)
NCCL_P2P_DISABLE=1 OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 --master_port=65354\
        pretrain_mae_vit.py \
        --data_path ${DATA_PATH} \
        --mask_type learnable \
        --mask_ratio 0.95 \
