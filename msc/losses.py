...
from msc import ramps


class ramp_weight(object):
    """
...
class ramp_softmax_temp(object):
    """
    ramp_types = ['sigmoid_rampup', 'linear_rampup', 'cosine_rampup', 'log_rampup', 'exp_rampup']
    """
    def __init__(self, start_w, final_w, iters_per_epoch, rampup_starts=0, rampup_ends=7, ramp_type='sigmoid_rampup'):
    ...
    def __call__(self, epoch, curr_iter):
        ...
        if cur_total_iter < self.rampup_starts:
            return 0
        self.current_rampup = self.rampup_func(cur_total_iter - self.rampup_starts, self.rampup_length)
        return self.start_w + (self.final_w-self.start_w) * self.current_rampup
...
def get_alpha(supervised_loader):
    ...
    num_labels = 0
    for imageA_batch, imageB_batch, label_batch in supervised_loader:
